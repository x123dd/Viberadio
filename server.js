...
// 生产环境静音调试日志（由 desktop/main.js 设置 VIBERADIO_SILENT_LOGS=1）
...
const UPDATE_WORK_DIR = process.env.VIBERADIO_UPDATE_DIR || path.join(__dirname, 'updates');
const UPDATE_DOWNLOAD_DIR = process.env.VIBERADIO_UPDATE_DOWNLOAD_DIR || path.join(UPDATE_WORK_DIR, 'downloads');
const UPDATE_PATCH_BACKUP_DIR = process.env.VIBERADIO_PATCH_BACKUP_DIR || path.join(UPDATE_WORK_DIR, 'backups', 'patches');
const BEATMAP_CACHE_DIR = process.env.VIBERADIO_BEAT_CACHE_DIR || path.join(__dirname, 'beatmaps');
const APP_PACKAGE = readPackageInfo();
const APP_VERSION = process.env.VIBERADIO_VERSION || APP_PACKAGE.version || '0.9.11';
...
const COOKIE_ENC_SALT = Buffer.from('viberadio-cookie-v1-salt');
...
function readUpdateConfig(pkg) {
  const local = (pkg && pkg.viberadio && pkg.viberadio.update) || {};
  const repoHint = process.env.VIBERADIO_UPDATE_REPOSITORY
  ...
  const owner = process.env.VIBERADIO_UPDATE_OWNER || local.owner || parsed.owner || '';
  const repo = process.env.VIBERADIO_UPDATE_REPO || local.repo || parsed.repo || '';
  return {
    ...
    manifest: process.env.VIBERADIO_UPDATE_MANIFEST
      || process.env.VIBERADIO_UPDATE_MANIFEST_URL
      || process.env.VIBERADIO_UPDATE_MANIFEST_FILE
      || '',
  };
}
...
function readUpdateMirrors(local) {
  const envMirrors = process.env.VIBERADIO_UPDATE_MIRRORS || process.env.VIBERADIO_UPDATE_MIRROR || '';
...
function normalizeManifestUpdateInfo(data) {
  ...
  const patchInfo = patch && patch.downloadUrl ? {
    name: patch.name || updateAssetNameFromUrl(patch.downloadUrl) || `Viberadio-${APP_VERSION}→${latestVersion}.patch.json`,
  ...
  const assetInfo = downloadUrl ? {
    name: asset.name || updateAssetNameFromUrl(downloadUrl) || `Viberadio-${latestVersion}-Setup.exe`,
  ...
  return {
    ...
    release: {
      ...
      name: release.name || data.name || ('Viberadio v' + latestVersion),
...
async function readUpdateManifest(ref) {
  ...
  if (/^https?:\/\//i.test(value)) {
    const resp = await fetch(value, {
      headers: { 'User-Agent': `Viberadio/${APP_VERSION}` },
    });
...
function parseLatestYmlUpdateInfo(text, reason) {
  ...
  const assetPath = yamlScalar(text, 'path') || yamlScalar(text, 'url') || `Viberadio-${latestVersion}-Setup.exe`;
  ...
  return {
    ...
    release: {
      ...
      name: 'Viberadio v' + latestVersion,
      version: latestVersion,
...
async function fetchLatestUpdateInfo() {
  ...
  try {
    const resp = await fetch(apiUrl, {
      ...
      headers: {
        'User-Agent': `Viberadio/${APP_VERSION}`,
    ...
    return {
      ...
      release: {
        ...
        name: data.name || ('Viberadio v' + latestVersion),
        version: latestVersion,
...
function safeUpdateFileName(name, version) {
  const raw = String(name || '').trim() || `Viberadio-${version || APP_VERSION}.exe`;
  const cleaned = raw
    .replace(/[<>:"/\\|?*\x00-\x1F]/g, '-')
    .replace(/\s+/g, ' ')
    .trim()
    .slice(0, 160);
  return cleaned || `Viberadio-${version || APP_VERSION}.exe`;
}
...
function writePatchFile(job, file) {
  ...
  const tmp = target + '.viberadio-patch';
...
function normalizePatchPayload(payload) {
  ...
  if (type && type !== 'viberadio-resource-patch') throw new Error('UNSUPPORTED_PATCH_TYPE');
...
const KUGOU_SONG_INFO_URL = 'https://wwwapi.kugou.com/yy/index.php?r=play/getdata';
const KUGOU_HEADERS = {
  Referer: 'https://www.kugou.com/',
  'User-Agent': UA,
};
const KUGOU_QUALITY_CANDIDATES = [
  { level: 'hires', br: 999000, label: '无损' },
  { level: 'exhigh', br: 320000, label: '高品质' },
  { level: 'standard', br: 128000, label: '标准' },
];
...
// ====================================================================
//  酷狗音乐 (KuGou Music) API
// ====================================================================

// ---------- 酷狗 Cookie 工具 ----------
function parseKgCookieString(raw) {
  const obj = {};
...
function kgCookieUin() {
  const obj = parseKgCookieString(kgCookie);
  return String(obj.userid || obj.kg_mid || obj.kg_m || '').replace(/\D/g, '') || '';
}
function kgCookieToken() {
  const obj = parseKgCookieString(kgCookie);
  return obj.token || obj.kg_mid_token || obj.kugou_token || '';
}
function kgCookieNickname() {
  const obj = parseKgCookieString(kgCookie);
  return decodeURIComponent(obj.nickname || obj.username || obj.kg_nick || '') || ('酷狗用户_' + (kgCookieUin().slice(0, 6) || '未知'));
}
function kgCookieHasLogin() {
  return !!(kgCookieUin() && kgCookieToken());
}

// ---------- 酷狗工具 ----------
function kgSongCover(hash, albumId, size) {
  const s = size || 400;
  if (hash) return `https://imge.kugou.com/stdmusic/${s}/${hash.slice(-4)}/${hash.slice(-2)}/${hash}/${s}.jpg`;
  if (albumId) return `https://imge.kugou.com/stdmusic/${s}/${albumId.slice(-4)}/${albumId.slice(-2)}/${albumId}/${s}.jpg`;
  return '';
}
function mapKuGouSong(raw) {
  raw = raw || {};
  const authors = Array.isArray(raw.authors) ? raw.authors : [];
  const hash = raw.hash || raw.song_hash || raw.HASH || '';
  const albumId = raw.album_id || raw.album_audio_id || raw.ALBUM_ID || '';
  const duration = raw.duration || raw.timelength || raw.DURATION || 0;
  return {
    provider: 'kugou',
    source: 'kugou',
    type: 'song',
    id: hash,
    name: raw.songname || raw.song_name || raw.SONGNAME || raw.name || '',
    artist: raw.singername || raw.singer_name || raw.SINGERNAME || (authors.map(a => a.author_name || a.singer_name).join(' / ')) || '',
    artists: authors.length ? authors.map(a => ({ id: a.author_id || '', name: a.author_name || a.singer_name || '' })) : [{ id: '', name: raw.singername || '' }],
    artistId: authors[0] && authors[0].author_id ? String(authors[0].author_id) : '',
    album: raw.album_name || raw.album || raw.ALBUMNAME || '',
    cover: raw.img || raw.cover || raw.cover_url || kgSongCover(hash, albumId),
    duration: typeof duration === 'number' && duration > 1000 ? duration : (duration * 1000),
    hash,
    album_id: albumId,
    fee: raw.fee || raw.pay_type || 0,
  };
}
function classifyKuGouPlaybackRestriction(info) {
  const msg = String((info && (info.msg || info.err_msg || info.message)) || '').toLowerCase();
  const code = Number((info && (info.status || info.code || info.errno)) || 0);
  if (!kgCookieHasLogin() && (code === 401 || code === 1001 || /未登录|请先登录|登录|权限不足/.test(msg))) {
    return { provider: 'kugou', category: 'login_required', action: 'login', message: '酷狗音乐需要登录后才能获取播放地址', code };
  }
  if (/vip|会员|付费|购买|数字专辑|pay|fee/.test(msg)) {
    return { provider: 'kugou', category: 'paid_required', action: 'upgrade', message: '酷狗歌曲需要会员或购买权限', code };
  }
  if (code && code !== 0) {
    return { provider: 'kugou', category: 'copyright_unavailable', action: 'switch_source', message: '酷狗版权暂不可播或需要官方客户端', code };
  }
  return { provider: 'kugou', category: 'url_unavailable', action: 'switch_source', message: '酷狗没有返回可播放地址', code };
}

// ---------- 酷狗搜索 ----------
async function kugouSearch(keywords, limit) {
  console.log('[KuGouSearch]', keywords, 'limit:', limit);
  const params = new URLSearchParams({
    keyword: keywords,
    page: '1',
    pagesize: String(Math.max(6, Math.min(50, limit || 18))),
    platform: 'WebFilter',
    format: 'json',
  });
  const url = KUGOU_SEARCH_URL + '?' + params.toString();
  const resp = await fetch(url, { headers: KUGOU_HEADERS });
  if (!resp.ok) throw new Error('KuGou search failed: ' + resp.status);
  const body = await resp.json();
  const rawList = (body && body.data && body.data.lists) || [];
  return rawList.map(mapKuGouSong).filter(s => s.id && s.name);
}

// ---------- 酷狗歌曲播放地址 ----------
async function kugouSongUrl(hash, albumId, qualityPreference) {
  if (!hash || !albumId) throw new Error('Missing hash or album_id');
  qualityPreference = normalizeQualityPreference(qualityPreference);
  const startIdx = KUGOU_QUALITY_CANDIDATES.findIndex(q => q.level === qualityPreference);
  const candidates = startIdx >= 0 ? KUGOU_QUALITY_CANDIDATES.slice(startIdx) : KUGOU_QUALITY_CANDIDATES;

  const params = new URLSearchParams({ hash, album_id: albumId, mid: kgCookieUin() || '0' });
  const url = KUGOU_SONG_INFO_URL + '&' + params.toString();
  const headers = Object.assign({}, KUGOU_HEADERS);
  if (kgCookie) headers.Cookie = kgCookie;

  const resp = await fetch(url, { headers });
  if (!resp.ok) throw new Error('KuGou song info request failed: ' + resp.status);
  const body = await resp.json();

  if (body.status !== 1 && body.errno !== 0) {
    return {
      provider: 'kugou',
      url: '',
      playable: false,
      restriction: classifyKuGouPlaybackRestriction(body),
      quality: { level: 'standard', br: 128000, label: '标准' },
      raw: body,
    };
  }
  ...
  let resolvedLevel = 'standard';

  // 尝试从备选音质列表中选择
  if (!playUrl && data.play_backup_url) {
    playUrl = data.play_backup_url;
    resolvedLevel = 'standard';
  }
  // 检查额外音质列表
  const extraUrls = data.extra_urls || data.urls || [];
  if (!playUrl && extraUrls.length) {
    for (const q of candidates) {
      const match = extraUrls.find(u => (u.bitrate || u.br) >= q.br * 0.8 || (u.type || u.quality || '').includes(q.level));
      if (match && match.url) {
        playUrl = match.url;
        resolvedLevel = q.level;
        break;
      }
    }
    if (!playUrl) {
      playUrl = extraUrls[0].url || '';
      resolvedLevel = 'standard';
    }
  }
  ...
  const br = data.bitrate || data.br || 128;
  return {
    provider: 'kugou',
    url: playUrl,
    playable: true,
    quality: { level: resolvedLevel, br, label: br >= 900 ? '无损' : (br >= 300 ? '高品质' : '标准') },
    raw: body,
  };
}

// ---------- 酷狗歌词 ----------
function kugouLyricHash(hash) {
  if (!hash || hash.length < 4) return { hash1: '0', hash2: '0' };
  const h = hash.toLowerCase();
  const idx = h.length >= 4 ? 4 : h.length;
  return { hash1: h.slice(0, idx), hash2: h };
}
async function kugouLyric(hash, albumId, songname, artist) {
  if (!hash && !albumId) return { provider: 'kugou', lyric: '', tlyric: '', source: 'kg-empty' };
  try {
    const headers = Object.assign({}, KUGOU_HEADERS);
    const lyricParams = new URLSearchParams({
      ver: '1', client: 'pc', fmt: 'lrc', charset: 'utf8',
      id: hash || '',
      accesskey: albumId || hash || '',
      keyword: (songname || '') + ' - ' + (artist || ''),
      duration: '240000',
    });
    const lyricUrl = 'https://lyrics.kugou.com/download?' + lyricParams.toString();
    const resp = await fetch(lyricUrl, { headers });
    if (!resp.ok) return { provider: 'kugou', lyric: '', tlyric: '', source: 'kg-empty' };
    const body = await resp.json();
    let lyricText = '';
    let transText = '';
    if (body && body.content) {
      // Base64 解码歌词内容
      try {
        lyricText = Buffer.from(body.content, 'base64').toString('utf8') || '';
      } catch (e) {
        lyricText = String(body.content || '');
      }
    }
    // 尝试获取翻译歌词
    if (body && body.transcontent) {
      try {
        transText = Buffer.from(body.transcontent, 'base64').toString('utf8') || '';
      } catch (e) {
        transText = String(body.transcontent || '');
      }
    }
    return {
      provider: 'kugou',
      id: hash || '',
      lyric: lyricText.replace(/\r\n/g, '\n').replace(/\r/g, '\n'),
      tlyric: transText.replace(/\r\n/g, '\n').replace(/\r/g, '\n'),
      yrc: '',
      qrc: '',
      roma: '',
      source: lyricText ? 'kg-lyrics' : 'kg-empty',
    };
  } catch (e) {
    console.warn('[KuGouLyric]', e.message);
    return { provider: 'kugou', lyric: '', tlyric: '', source: 'kg-error' };
  }
}

// ---------- 酷狗登录态查询 ----------
async function getKGLoginInfo() {
  if (!kgCookie || !kgCookieHasLogin()) {
    return { provider: 'kugou', loggedIn: false, userId: '', nickname: '酷狗音乐', avatar: '', vipType: 0 };
  }
  const uin = kgCookieUin();
  const nickname = kgCookieNickname();
  const avatar = `https://imge.kugou.com/kugouicon/165/${uin.slice(-2)}/${uin}/${uin}.png`;

  // 尝试获取VIP信息
  let vipType = 0, isVip = false, vipLabel = '无VIP';
  try {
    const resp = await fetch('https://vipuser.kugou.com/v2/user/vipinfo?userid=' + uin, {
      headers: Object.assign({}, KUGOU_HEADERS, { Cookie: kgCookie }),
    });
    if (resp.ok) {
      const body = await resp.json();
      if (body.status === 1 && body.data) {
        vipType = body.data.vip_type || body.data.vipType || 0;
        isVip = vipType > 0;
        vipLabel = vipType >= 2 ? 'SVIP' : (vipType >= 1 ? 'VIP' : '无VIP');
      }
    }
  } catch (e) { /* ignore VIP check error */ }
  ...
  return {
    ...
    isVip,
    isSvip: vipType >= 2,
    vipLevel: vipType >= 2 ? 'svip' : (vipType >= 1 ? 'vip' : 'none'),
    vipLabel,
    hasCookie: !!kgCookie,
  };
}
...
async function handleKGUserPlaylists() {
  ...
  if (!info.loggedIn) return { provider: 'kugou', loggedIn: false, playlists: [] };
  try {
    const headers = Object.assign({}, KUGOU_HEADERS, { Cookie: kgCookie });
    const resp = await fetch('https://kollection.kugou.com/v3/collection/getUserSpecialList?userid=' + info.userId + '&page=1&pagesize=60', { headers });
    if (!resp.ok) return { provider: 'kugou', loggedIn: true, playlists: [] };
    const body = await resp.json();
    const rawList = (body && body.data && body.data.lists) || [];
    const playlists = rawList.map(pl => ({
      provider: 'kugou',
      id: pl.specialid || pl.id || '',
      name: pl.specialname || pl.name || '',
      cover: pl.img || pl.cover || pl.imgurl || '',
      trackCount: pl.songcount || pl.trackCount || 0,
      playCount: pl.playcount || pl.playCount || 0,
      creator: pl.nickname || pl.username || '',
      subscribed: !!pl.subscribed,
    }));
    return { provider: 'kugou', loggedIn: true, userId: info.userId, playlists };
  } catch (e) {
    console.warn('[KuGouPlaylists]', e.message);
    return { provider: 'kugou', loggedIn: true, playlists: [] };
  }
}

// ---------- 酷狗歌单曲目 ----------
async function handleKGPlaylistTracks(specialId) {
  if (!specialId) throw new Error('Missing playlist id');
  const headers = Object.assign({}, KUGOU_HEADERS);
  if (kgCookie) headers.Cookie = kgCookie;
  const resp = await fetch(`https://kollection.kugou.com/v3/collection/getSpecialSong?specialid=${specialId}&page=1&pagesize=200`, { headers });
  if (!resp.ok) throw new Error('KuGou playlist tracks failed: ' + resp.status);
  const body = await resp.json();
  const rawList = (body && body.data && body.data.lists) || (body && body.data && body.data.info) || [];
  return (Array.isArray(rawList) ? rawList : []).map(mapKuGouSong).filter(s => s.id && s.name);
}

// ---------- 汽水音乐 (Qishui / Douyin Music) ----------
const QISHUI_SEARCH_URL = 'https://api.drcn.music.douyin.com/v1/search';
const QISHUI_SONG_URL = 'https://api.drcn.music.douyin.com/v1/song';
const QISHUI_HEADERS = {
  'User-Agent': UA,
  'Accept': 'application/json',
  'Referer': 'https://qishui.douyin.com/',
};
const QISHUI_QUALITY_CANDIDATES = [
  { level: 'lossless', br: 921600, label: '无损' },
  { level: 'exhigh', br: 320000, label: 'HQ' },
  { level: 'standard', br: 128000, label: '标准' },
];
...
function qishuiCookieObj() {
  if (!qishuiCookie) return {};
  return parseCookieStringForQishui(qishuiCookie);
}
function qishuiCookieUin() {
  const obj = qishuiCookieObj();
  return obj.douyin_uid || obj.uid || obj.user_id || '';
}
function qishuiCookieSessionId() {
  const obj = qishuiCookieObj();
  return obj.sessionid || obj.session_id || obj.SESSIONID || '';
}
function qishuiCookieNickname() {
  const obj = qishuiCookieObj();
  return decodeURIComponent(obj.nickname || obj.user_name || obj.douyin_nickname || '') || '汽水音乐';
}
function qishuiCookieHasLogin() {
  return !!(qishuiCookieUin() && qishuiCookieSessionId());
}

function mapQishuiSong(raw) {
  raw = raw || {};
  const album = raw.album || raw.albumInfo || {};
  const artist = raw.artist || raw.singer || raw.author || {};
  const artists = raw.artists || raw.singers || (artist ? [artist] : []);
  const artistName = (Array.isArray(artists) ? artists.map(a => a.name || a.artist_name || a.singer_name || '').filter(Boolean).join('/') : '') || artist.name || artist.artist_name || artist.singer_name || '';
  const albumName = album.name || album.album_name || album.title || '';
  const albumId = album.id || album.album_id || album.albumId || '';
  const coverUrl = (album.cover || album.cover_url || album.pic_url || raw.cover || raw.cover_url || '').replace(/^http:/, 'https:');
  const id = raw.id || raw.song_id || raw.songId || '';
  const mid = raw.mid || raw.song_mid || id;
  const duration = raw.duration || raw.duration_ms || raw.play_duration || 0;

  return {
    id,
    mid,
    name: raw.name || raw.title || raw.song_name || '',
    artist: artistName,
    album: albumName,
    albumId,
    cover: coverUrl,
...
function classifyQishuiPlaybackRestriction(body) {
  if (!body) return 'unavailable';
  const code = body.code || body.status_code || body.errcode || 0;
  if (code === 0 || code === 200 || body.url) return 'none';
  if (code === 401 || code === 403) return 'login_required';
  if (code === 404) return 'not_found';
  return body.message || body.msg || 'unavailable';
}

async function qishuiSearch(keywords, limit) {
  console.log('[QishuiSearch]', keywords, 'limit:', limit);
  const params = new URLSearchParams({
    keyword: keywords,
    limit: String(Math.max(6, Math.min(30, limit || 18))),
    offset: '0',
  });
  const url = QISHUI_SEARCH_URL + '?' + params.toString();
  const headers = Object.assign({}, QISHUI_HEADERS);
  if (qishuiCookie) headers.Cookie = qishuiCookie;
  const resp = await fetch(url, { headers });
  if (!resp.ok) throw new Error('Qishui search failed: ' + resp.status);
  const body = await resp.json();
  const rawList = (body && body.data && body.data.songs) || (body && body.songs) || (body && body.data && body.data.list) || (body && body.data) || [];
  const list = Array.isArray(rawList) ? rawList : (rawList.list || rawList.items || []);
  return list.map(mapQishuiSong).filter(s => s.id && s.name);
}

async function qishuiSongUrl(songId, qualityPreference) {
  if (!songId) throw new Error('Missing song id');
  qualityPreference = normalizeQualityPreference(qualityPreference);
  const startIdx = QISHUI_QUALITY_CANDIDATES.findIndex(q => q.level === qualityPreference);
  const candidates = startIdx >= 0 ? QISHUI_QUALITY_CANDIDATES.slice(startIdx) : QISHUI_QUALITY_CANDIDATES;

  const headers = Object.assign({}, QISHUI_HEADERS);
  if (qishuiCookie) headers.Cookie = qishuiCookie;

  const resp = await fetch(`${QISHUI_SONG_URL}/${encodeURIComponent(songId)}/url`, { headers });
  if (!resp.ok) throw new Error('Qishui song url failed: ' + resp.status);
  const body = await resp.json();

  if (body.code !== 0 && body.status_code !== 200) {
    return {
      provider: 'qishui',
      url: '',
      playable: false,
      restriction: classifyQishuiPlaybackRestriction(body),
      quality: { level: 'standard', br: 128000, label: '标准' },
      raw: body,
    };
  }
  ...
  let playUrl = data.url || data.play_url || data.stream_url || '';
  let br = data.bitrate || data.br || data.quality_br || 128000;
  let resolvedLevel = 'standard';

  if (!playUrl && data.urls && Array.isArray(data.urls)) {
    for (const q of candidates) {
      const match = data.urls.find(u => (u.bitrate || u.br) >= q.br * 0.8);
      if (match && match.url) {
        playUrl = match.url;
        resolvedLevel = q.level;
        br = match.bitrate || match.br || q.br;
        break;
      }
    }
    if (!playUrl && data.urls[0] && data.urls[0].url) {
  ...
  return {
    ...
    url: playUrl,
    playable: true,
    quality: { level: resolvedLevel, br, label: br >= 900000 ? '无损' : (br >= 300000 ? 'HQ' : '标准') },
    raw: body,
  };
}

async function qishuiLyric(songId) {
  if (!songId) return { provider: 'qishui', lyric: '', tlyric: '', source: 'qs-empty' };
  try {
    const headers = Object.assign({}, QISHUI_HEADERS);
    if (qishuiCookie) headers.Cookie = qishuiCookie;
    const resp = await fetch(`${QISHUI_SONG_URL}/${encodeURIComponent(songId)}/lyric`, { headers });
    if (!resp.ok) return { provider: 'qishui', lyric: '', tlyric: '', source: 'qs-empty' };
    const body = await resp.json();
    const data = body.data || body;
    const lyricText = data.lyric || data.lrc || data.lyric_text || '';
    const transText = data.tlyric || data.trans_lyric || data.translated_lyric || '';
    return { provider: 'qishui', lyric: lyricText, tlyric: transText, source: 'qishui' };
  } catch (e) {
    console.warn('[QishuiLyric]', e.message);
    return { provider: 'qishui', lyric: '', tlyric: '', source: 'qs-error' };
  }
}

async function getQishuiLoginInfo() {
  if (!qishuiCookie || !qishuiCookieHasLogin()) {
    return { provider: 'qishui', loggedIn: false, userId: '', nickname: '汽水音乐', avatar: '', vipType: 0 };
  }
  const uin = qishuiCookieUin();
  const nickname = qishuiCookieNickname();
  const avatar = `https://p3-pc.douyinpic.com/aweme/100x100/${uin}.jpeg`;

  let vipType = 0, isVip = false, vipLabel = '无VIP';
  try {
    const headers = Object.assign({}, QISHUI_HEADERS);
    if (qishuiCookie) headers.Cookie = qishuiCookie;
    const resp = await fetch(`https://api.drcn.music.douyin.com/v1/user/${uin}/vip`, { headers });
    if (resp.ok) {
      const body = await resp.json();
      const vipData = body.data || body;
      if (vipData && vipData.is_vip) {
        vipType = (vipData.vip_level || vipData.level || 1);
        isVip = true;
        vipLabel = vipType >= 2 ? 'SVIP' : 'VIP';
      }
    }
  } catch (e) { /* ignore VIP check error */ }
  ...
  return {
    ...
    vipLevel: vipType >= 2 ? 'svip' : (vipType >= 1 ? 'vip' : 'none'),
    vipLabel,
    hasCookie: !!qishuiCookie,
  };
}

async function handleQishuiUserPlaylists() {
  const info = await getQishuiLoginInfo();
  if (!info.loggedIn) return { provider: 'qishui', loggedIn: false, playlists: [] };
  try {
    const headers = Object.assign({}, QISHUI_HEADERS);
    if (qishuiCookie) headers.Cookie = qishuiCookie;
    const resp = await fetch(`https://api.drcn.music.douyin.com/v1/user/${info.userId}/playlists?limit=60`, { headers });
    if (!resp.ok) return { provider: 'qishui', loggedIn: true, playlists: [] };
    const body = await resp.json();
    const rawList = (body && body.data && body.data.playlists) || (body && body.data) || [];
    const list = Array.isArray(rawList) ? rawList : (rawList.playlists || rawList.items || []);
    const playlists = list.map(pl => ({
      provider: 'qishui',
      id: pl.id || pl.playlist_id || '',
      name: pl.name || pl.title || '',
      cover: (pl.cover || pl.cover_url || pl.pic_url || '').replace(/^http:/, 'https:'),
      trackCount: pl.track_count || pl.song_count || pl.total || 0,
      playCount: pl.play_count || pl.listen_count || 0,
      creator: pl.creator_name || pl.nickname || '',
      subscribed: !!pl.subscribed,
    }));
    return { provider: 'qishui', loggedIn: true, userId: info.userId, playlists };
  } catch (e) {
    console.warn('[QishuiPlaylists]', e.message);
    return { provider: 'qishui', loggedIn: true, playlists: [] };
  }
}

async function handleQishuiPlaylistTracks(playlistId) {
  if (!playlistId) throw new Error('Missing playlist id');
  const headers = Object.assign({}, QISHUI_HEADERS);
  if (qishuiCookie) headers.Cookie = qishuiCookie;
  const resp = await fetch(`https://api.drcn.music.douyin.com/v1/playlist/${encodeURIComponent(playlistId)}/tracks?limit=200`, { headers });
  if (!resp.ok) throw new Error('Qishui playlist tracks failed: ' + resp.status);
  const body = await resp.json();
  const rawList = (body && body.data && body.data.tracks) || (body && body.data) || [];
  const list = Array.isArray(rawList) ? rawList : (rawList.tracks || rawList.items || []);
  return list.map(mapQishuiSong).filter(s => s.id && s.name);
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (pn === '/api/app/version') {
    sendJSON(res, {
      name: APP_PACKAGE.name || 'viberadio',
      productName: APP_PACKAGE.productName || 'Viberadio',
      version: APP_VERSION,
      update: {
  ...
  if (pn === '/api/kg/search') {
    try {
      ...
      const limit = Math.max(6, Math.min(50, parseInt(url.searchParams.get('limit') || '18', 10) || 18));
  ...
  if (pn === '/api/kg/song/url') {
    try {
      ...
      const albumId = String(url.searchParams.get('album_id') || url.searchParams.get('albumId') || '').trim();
      const quality = String(url.searchParams.get('quality') || 'hires').trim();
      if (!hash || !albumId) { sendJSON(res, { provider: 'kugou', error: 'Missing hash or album_id', playable: false }, 400); return; }
      const data = await kugouSongUrl(hash, albumId, quality);
      sendJSON(res, data);
    } catch (err) {
      console.error('[KuGouSongUrl]', err);
      sendJSON(res, { provider: 'kugou', error: err.message, playable: false }, 500);
    }
    return;
  }
  ...
  if (pn === '/api/kg/lyric') {
    try {
      const hash = String(url.searchParams.get('hash') || '').trim();
      const albumId = String(url.searchParams.get('album_id') || url.searchParams.get('albumId') || '').trim();
      const songname = String(url.searchParams.get('songname') || url.searchParams.get('name') || '').trim();
      const artist = String(url.searchParams.get('artist') || '').trim();
      if (!hash && !albumId) { sendJSON(res, { provider: 'kugou', lyric: '', tlyric: '', source: 'kg-empty' }, 400); return; }
      const data = await kugouLyric(hash, albumId, songname, artist);
      sendJSON(res, data);
    } catch (err) {
  ...
  if (pn === '/api/kg/login/cookie') {
    try {
      ...
      const raw = body.cookie || body.token || body.data || '';
      if (!raw) { sendJSON(res, { provider: 'kugou', loggedIn: false, error: 'Missing cookie' }, 400); return; }
      const normalized = normalizeCookieHeader(raw) || rawCookieFallback(raw);
      const obj = parseKgCookieString(normalized);
      const uid = String(obj.userid || obj.kg_mid || obj.kg_m || '').replace(/\D/g, '') || '';
      const token = obj.token || obj.kg_mid_token || obj.kugou_token || '';
      if (!uid || !token) {
        sendJSON(res, { provider: 'kugou', loggedIn: false, error: 'INVALID_KG_COOKIE', message: '酷狗 cookie 缺少 userid 或 token' }, 400);
        return;
      }
  ...
  if (pn === '/api/kg/login/status') {
    try {
      ...
      sendJSON(res, { provider: 'kugou', loggedIn: false, error: err.message });
    }
    return;
  }

  if (pn === '/api/kg/logout') {
    try { saveKGCookie(''); } catch (e) {}
    sendJSON(res, { provider: 'kugou', ok: true });
    return;
  }
  ...
  if (pn === '/api/kg/user/playlists') {
    try {
      ...
      sendJSON(res, { provider: 'kugou', error: err.message, loggedIn: false, playlists: [] }, 500);
    }
    return;
  }
  ...
  if (pn === '/api/kg/playlist/tracks') {
    try {
      ...
      if (!id) { sendJSON(res, { provider: 'kugou', error: 'Missing playlist id', songs: [] }, 400); return; }
      const songs = await handleKGPlaylistTracks(id);
      sendJSON(res, { provider: 'kugou', songs });
    } catch (err) {
  ...
  if (pn === '/api/qs/search') {
    try {
      const kw = url.searchParams.get('keywords') || '';
      const limit = Math.max(4, Math.min(24, parseInt(url.searchParams.get('limit') || '12', 10) || 12));
      const songs = await qishuiSearch(kw, limit);
  ...
  if (pn === '/api/qs/song/url') {
    try {
      const id = url.searchParams.get('id') || url.searchParams.get('songId') || '';
      const quality = url.searchParams.get('quality') || '';
      const info = await qishuiSongUrl(id, quality);
      sendJSON(res, info);
    } catch (err) {
  ...
  if (pn === '/api/qs/lyric') {
    try {
      const id = url.searchParams.get('id') || url.searchParams.get('songId') || '';
      if (!id) { sendJSON(res, { provider: 'qishui', error: 'Missing song id', lyric: '' }, 400); return; }
      const data = await qishuiLyric(id);
      sendJSON(res, data);
    } catch (err) {
  ...
  if (pn === '/api/qs/login/cookie') {
    try {
      const body = await readRequestBody(req);
      const raw = body.cookie || body.data || body.text || '';
      const normalized = normalizeCookieHeader(raw) || rawCookieFallback(raw);
      const obj = parseCookieStringForQishui(normalized);
      if (!(obj.douyin_uid || obj.uid || obj.user_id) || !(obj.sessionid || obj.session_id || obj.SESSIONID)) {
        sendJSON(res, { provider: 'qishui', loggedIn: false, error: 'INVALID_QISHUI_COOKIE', message: '汽水音乐 cookie 缺少 uid 或 sessionid' }, 400);
        return;
      }
  ...
  if (pn === '/api/qs/user/playlists') {
    try {
      const data = await handleQishuiUserPlaylists();
      sendJSON(res, data);
    } catch (err) {
      console.error('[QishuiUserPlaylists]', err);
      sendJSON(res, { provider: 'qishui', loggedIn: false, error: err.message, playlists: [] }, 500);
    }
    return;
  }

  if (pn === '/api/qs/playlist/tracks') {
    try {
      const id = url.searchParams.get('id') || url.searchParams.get('playlistId') || '';
      const data = await handleQishuiPlaylistTracks(id);
      sendJSON(res, data);
    } catch (err) {
      console.error('[QishuiPlaylistTracks]', err);
      sendJSON(res, { provider: 'qishui', error: err.message, tracks: [] }, 500);
    }
    return;
  }
  ...
  if (pn === '/api/cover') {
    try {
      ...
      if (!coverUrl || !/^https?:\/\//i.test(coverUrl)) {
        res.writeHead(400, { 'Access-Control-Allow-Origin': 'http://127.0.0.1:' + PORT });
...
server.listen(PORT, HOST, () => {
  console.log('======================================================');
  console.log(' Viberadio v2  →  http://localhost:' + PORT);
