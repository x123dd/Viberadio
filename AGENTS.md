# Viberadio Project Rules

## Project Identity

Viberadio 是 Windows Electron 桌面音乐播放器，核心体验包括搜索、播放、歌单、歌词、3D 歌单架、粒子视觉预设、DIY 视觉控制台和 GitHub 自动更新。

- 当前可运行程序：`E:\桌面\播放器软件\Viberadio\Viberadio.exe`
- 当前运行版主目录：`E:\桌面\播放器软件\Viberadio`
- 当前真实代码/Git 仓库：`E:\桌面\播放器软件\Viberadio\resources\app`
...
新对话开始处理 Viberadio 前，必须先确认当前目录是：

```powershell
E:\桌面\播放器软件\Viberadio\resources\app
```
...
前端主逻辑在 `public/index.html`。这个目录是正在运行的 `Viberadio.exe` 使用的 app 目录，所以改完后重启外层 `E:\桌面\播放器软件\Viberadio\Viberadio.exe` 就能及时检查效果。没有独立 npm test，改动后至少做：

注意：运行版 `resources\app\node_modules` 可能只包含运行依赖。如果发布打包时缺少 `electron-builder`，先在 `E:\桌面\播放器软件\Viberadio\resources\app` 执行 `npm install`，再执行 `npm run build:win`。
...
5. 上传 GitHub Release 资产：
   - `dist/Viberadio-x.y.z-Setup.exe`
   - `dist/Viberadio-x.y.z-Setup.exe.blockmap`
   - `dist/latest.yml`
   - 需要的 `Viberadio-旧版本-x.y.z.json` 轻量补丁
...
- 重要：不要再改旧外层源码目录。旧的 `E:\桌面\播放器软件\Viberadio\public` / `desktop` 已经归档；现在只有 `E:\桌面\播放器软件\Viberadio\resources\app\public` / `desktop` 会影响运行版。
