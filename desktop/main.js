...
async function readKgLoginCookieHeader(cookieSession) {
  const cookies = await cookieSession.cookies.get({});
  return buildCookieHeaderFor(cookies, isKuGouCookieDomain, []);
}

function isQishuiCookieDomain(domain) {
  const normalized = String(domain || '').replace(/^\./, '').toLowerCase();
  return normalized === 'douyin.com' || normalized.endsWith('.douyin.com') ||
    normalized === 'qishui.douyin.com' || normalized.endsWith('.qishui.douyin.com');
}
...
async function readQishuiLoginCookieHeader(cookieSession) {
  const cookies = await cookieSession.cookies.get({});
  return buildCookieHeaderFor(cookies, isQishuiCookieDomain, []);
}
    ...
    loginWindow.webContents.on('did-finish-load', () => {
      ...
      loginWindow.webContents.executeJavaScript(`
        setTimeout(() => {
          ...
          const loginNode = nodes.find((node) => {
            const text = (node.textContent || '').trim();
            if (!/登录|登錄|登陆|Login|Sign\\s*In/i.test(text)) return false;
            const rect = node.getBoundingClientRect();
            return rect.width > 0 && rect.height > 0;
          });
          if (loginNode) loginNode.click();
...
async function clearKGMusicLoginSession() {
  const cookieSession = session.fromPartition(KG_LOGIN_PARTITION);
...
ipcMain.handle('kg-music-open-login', async (event) => {
  return openKgMusicLoginWindow(getSenderWindow(event));
});

ipcMain.handle('kg-music-clear-login', async () => {
  return clearKGMusicLoginSession();
});
...
async function createWindow() {
  ...
  try {
    const legacyKGCookie = path.join(__dirname, '..', '.kg-cookie');
    if (fs.existsSync(legacyKGCookie)) {
      if (!fs.existsSync(process.env.KUGOU_COOKIE_FILE)) {
        fs.copyFileSync(legacyKGCookie, process.env.KUGOU_COOKIE_FILE);
      }
      fs.unlinkSync(legacyKGCookie);
    }
  } catch (e) {
