...
function createDecodeState(hopSec) {
  const state = {
    lowEnergy: [],
    hitEnergy: [],
    hp: null,
  ...
  function initFilters(sampleRate) {
    if (state.effectiveSr) return;
    state.sampleStep = sampleRate >= 44100 ? 4 : (sampleRate >= 32000 ? 3 : 2);
    state.effectiveSr = sampleRate / state.sampleStep;
    state.hopSize = Math.max(80, Math.floor(state.effectiveSr * hopSec));
    state.hp = makeBiquad('highpass', 32, 0.72, state.effectiveSr);
    state.lp = makeBiquad('lowpass', 178, 0.82, state.effectiveSr);
  }

  function pushFrame() {
    const count = Math.max(1, state.frameCount);
    state.lowEnergy.push(Math.sqrt(state.frameSum / count));
    state.hitEnergy.push(state.framePeak);
    state.frameSum = 0;
    state.framePeak = 0;
    state.frameCount = 0;
  }
  ...
  function processDecoded(result) {
    ...
    state.decodedSamples += n;
    for (let i = 0; i < n; i += state.sampleStep) {
      const x = right ? ((left[i] || 0) + (right[i] || 0)) * 0.5 : (left[i] || 0);
      const y = runBiquad(state.lp, runBiquad(state.hp, x));
      const ay = Math.abs(y);
      state.frameSum += y * y;
      if (ay > state.framePeak) state.framePeak = ay;
      state.frameCount++;
      state.effectiveSamples++;
      if (state.frameCount >= state.hopSize) pushFrame();
    }
  }

  return { state, initFilters, pushFrame, processDecoded };
}
...
async function decodePodcastDjEnergyRange(audioUrl, opts) {
  ...
  const ctx = createDecodeState(hopSec);
  ...
  try {
    ...
    while (true) {
      ...
      ctx.state.chunks++;
      ctx.processDecoded(decoder.decode(value instanceof Uint8Array ? value : new Uint8Array(value)));
      if (limitSec && ctx.state.effectiveSr && ctx.state.effectiveSamples / ctx.state.effectiveSr >= limitSec) {
        try { await reader.cancel(); } catch (e) {}
        break;
      }
      if (ctx.state.chunks % 12 === 0) await new Promise(resolve => setImmediate(resolve));
    }
    ctx.processDecoded(decoder.decode(new Uint8Array(0)));
    if (ctx.state.frameCount > 0) ctx.pushFrame();
  } finally {
    decoder.free();
  }

  return {
    lowEnergy: ctx.state.lowEnergy,
    hitEnergy: ctx.state.hitEnergy,
    hopSec,
    duration: ctx.state.effectiveSr ? ctx.state.effectiveSamples / ctx.state.effectiveSr : 0,
    decode: {
      chunks: ctx.state.chunks,
      decodedSamples: ctx.state.decodedSamples,
      sampleRate: ctx.state.effectiveSr ? ctx.state.effectiveSr * ctx.state.sampleStep : 0,
      effectiveSampleRate: ctx.state.effectiveSr,
      frames: ctx.state.lowEnergy.length,
    },
  };
}
...
async function analyzePodcastDjStreamFull(audioUrl, opts) {
  ...
  try {
    ...
    ctx.processDecoded(tail);
    if (ctx.state.frameCount > 0) ctx.pushFrame();
  } finally {
    decoder.free();
  }

  const effectiveDuration = ctx.state.effectiveSr ? ctx.state.effectiveSamples / ctx.state.effectiveSr : 0;
  const duration = effectiveDuration || durationHint;
  const map = buildBeatMapFromLowEnergy(ctx.state.lowEnergy, ctx.state.hitEnergy, hopSec, duration);
  map.decode = {
    chunks: ctx.state.chunks,
    decodedSamples: ctx.state.decodedSamples,
    sampleRate: ctx.state.effectiveSr ? ctx.state.effectiveSr * ctx.state.sampleStep : 0,
