...
`v1.0.10` 及更早旧安装包不再建议继续安装或传播。请将旧 `.exe` 安装包视为不可信历史产物并隔离保留；需要安装 Viberadio 时，请使用 `v1.1.0` 或更新版本的 GitHub Release 安装包。
...
Viberadio 不应收集或上传用户 Cookie。用户登录状态应保存在本地用户数据目录中。
